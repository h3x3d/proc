...
const IFACE_FIELDS = [...NET_FIELDS.map(v => `rx_${ v }`), ...NET_FIELDS.slice(0, -1).map(v => `tx_${ v }`)];

const DISK_FIELDS = ['device_number', 'device_number_minor', 'device', 'reads_completed', 'reads_merged', 'sectors_read', 'ms_reading', 'writes_completed', 'writes_merged', 'sectors_written', 'ms_writing', 'ios_pending', 'ms_io', 'ms_weighted_io'];
...
function Proc(basePath) {
  ...
  let disk = (() => {
    var ref = (0, _asyncToGenerator3.default)(function* () {
      const data = (yield readFile((0, _path.join)(basePath, 'diskstats'))).split(`\n`);
      const ret = {};

      for (const line of data.filter(function (v) {
        return v.length > 0;
      })) {
        const lineData = zip(DISK_FIELDS, line.split(/\s+/).filter(function (v) {
          return v.length > 0;
        }), function (v, k) {
          return k === 'device' ? v : parseInt(v, 10);
        });
  ...
  let net = (() => {
    var ref = (0, _asyncToGenerator3.default)(function* () {
      ...
      for (const line of data.filter(function (v) {
        ...
        var _line$trim$split2 = (0, _toArray3.default)(_line$trim$split);

        const iface = _line$trim$split2[0];

        const lineVals = _line$trim$split2.slice(1);
